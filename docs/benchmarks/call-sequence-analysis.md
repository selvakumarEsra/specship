...
The [A/B matrix](specship-ab-matrix.md) showed specship cuts **reads 75%** but **wall-clock only
...
4. **Round-trips are 25% fewer with specship (283 vs 375 turns)** but wall-clock is only 16%
   faster — because the with-arm's turns each carry a ~18K explore payload, inflating TTFT and
   eroding the turn savings.
5. **Root cause:** `src/mcp/server-instructions.ts` leads with *"answer directly … `specship_context`
   first, then ONE `specship_explore`"* as the headline pattern. The trace-first guidance is buried
...
| cells that called `specship_trace` | **3** (`cpp-leveldb`, `excalidraw`, `c-redis`) |
...
Across all cells, per specship tool — call count and **average payload per call**:
...
in this harness), a fixed 1-turn tax before any specship call. Worth confirming whether the
production install defers specship tools the same way.
...
- **Change:** rewrite the `server-instructions.ts` headline so a *flow* question (how does X reach Y
  / trace / from→to) routes to `specship_trace` **first**, demoting the context→explore pattern to
  non-flow/onboarding questions. Mirror into `instructions-template.ts` + `.cursor/rules/specship.mdc`.
...
`SPECSHIP_MCP_TOOLS` allowlist (so an ablated tool is genuinely absent from ListTools, not
...
2. **Strengthen the steering.** Arm A (shipped server-instructions, which *already* say "trace first
   ...
   `instructions-template.ts` + `.cursor/rules/specship.mdc` (house rule: all three together),
...
- **SHIP: the `SPECSHIP_MCP_TOOLS` allowlist** — independent, clean, validated.
...
- **SHIP: the `SPECSHIP_MCP_TOOLS` allowlist** — independent, validated.
...
same queries, **median of 4 runs/arm** (headless: specship-only MCP vs empty MCP):
...
cache-ability**: specship answers in far fewer turns over a much smaller accumulated context, while
