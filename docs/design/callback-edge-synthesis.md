...
> **Update (2026-06-01):** the `specship_trace` and `specship_context` MCP tools
> were since **removed** — `specship_explore` is the single surfacing tool now. Its
> "Flow" section (`buildFlowFromNamedSymbols`) and the `specship_node` trail surface
> these synthesized edges; the `trace(a, b)` notation below means "the a→b flow,"
> which you now verify with `specship_explore` / `probe-explore.mjs` (the
...
```bash
npm run build
rm -rf /tmp/specship-corpus/excalidraw/.specship
( cd /tmp/specship-corpus/excalidraw && specship init -i )
# synthesized edges (provenance='heuristic', metadata.synthesizedBy in {callback,event-emitter}):
sqlite3 /tmp/specship-corpus/excalidraw/.specship/specship.db \
...
node scripts/agent-eval/probe-explore.mjs /tmp/specship-corpus/excalidraw "triggerUpdate triggerRender"
```
...
- **Retrieval/UX changes** (separate from coverage): `explore` whole-small-file + glue
  ...
  — all in `src/mcp/tools.ts`. (`specship_trace` / `specship_context` were later
  removed; explore is the one surfacing tool.)
- **Full investigation context + findings:** auto-memory
  `project_specship_read_displacement` (why coverage — not prompting/hooks/new-tools —
  is the lever for getting agents to use specship over Read).
