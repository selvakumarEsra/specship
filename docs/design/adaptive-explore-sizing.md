# Design + status: adaptive `specship_explore` sizing (sibling skeletonization)
...
"Refinement" below). Escape hatch: `SPECSHIP_ADAPTIVE_EXPLORE=0`.
**Motivation:** make `specship_explore` size its output to the *answer* rather
...
`specship_explore` returned full source for **every** relevant file up to its
...
the rest of the session, the WITH-SpecShip arm cost *more* than the WITHOUT arm
...
A file is skeletonized iff **all** hold (and `SPECSHIP_ADAPTIVE_EXPLORE != 0`):
...
Headless `claude -p`, Opus 4.8, **WITH vs WITHOUT** SpecShip (the real benchmark
