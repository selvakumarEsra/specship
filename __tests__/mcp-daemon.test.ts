 ...
 *   - A stale lockfile (dead pid) is cleared; `SPECSHIP_NO_DAEMON=1` opts out;
 ...
 *
 * These tests intentionally spawn real `node dist/bin/specship.js` processes
 * over real sockets/pipes — the same surface a Claude Code / Cursor / Codex
 * install exercises. The daemon logs to `.specship/daemon.log` (it has no
...
function readLockPid(root: string): number | null {
  try {
    const raw = fs.readFileSync(path.join(root, '.specship', 'daemon.pid'), 'utf8');
...
function readDaemonLog(root: string): string {
  try { return fs.readFileSync(path.join(root, '.specship', 'daemon.log'), 'utf8'); }
  catch { return ''; }
}

function countListeningLines(root: string): number {
  return readDaemonLog(root).split('\n').filter((l) => l.includes('[SpecShip daemon] Listening on')).length;
}
...
describe('Shared MCP daemon (issue #411)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-mcp-daemon-'));
    const cg = await SpecShip.init(tempDir);
    cg.close();
  ...
  it('two invocations share ONE detached daemon; both attach as proxies', async () => {
    const env = { SPECSHIP_DAEMON_IDLE_TIMEOUT_MS: '15000' };
    ...
    expect(firstResp.result.serverInfo.name).toBe('specship');
    ...
    await waitFor(() => fs.existsSync(path.join(realRoot, '.specship', 'daemon.pid')), 8000);
    ...
    expect(secondResp.result.serverInfo.name).toBe('specship');
  ...
  it('concurrent launchers converge on a single daemon (lockfile race — must-fix 1)', async () => {
    ...
    for (let i = 0; i < procs.length; i++) {
      ...
      expect(resp.result.serverInfo.name).toBe('specship');
    }
  ...
  it('daemon survives the first client dying; a second client keeps working (must-fix 2 / #277)', async () => {
    ...
    const env = { SPECSHIP_DAEMON_IDLE_TIMEOUT_MS: '30000', SPECSHIP_PPID_POLL_MS: '200' };
  ...
  it('SPECSHIP_NO_DAEMON=1 keeps each process independent (no socket/pidfile)', async () => {
    const env = { SPECSHIP_NO_DAEMON: '1' };
    const first = spawnServer(tempDir, env);
    servers.push(first);
    sendInitialize(first.child, `file://${tempDir}`, 1);
    await waitFor(() => findResponse(first.stdout, 1), 10000);
    ...
    expect(fs.existsSync(path.join(realRoot, '.specship', 'daemon.pid'))).toBe(false);
    expect(fs.existsSync(path.join(realRoot, '.specship', 'daemon.log'))).toBe(false);
  }, 20000);
  ...
  it('clears a stale (dead-pid) lockfile and a fresh daemon takes over', async () => {
    ...
    fs.writeFileSync(
      path.join(realRoot, '.specship', 'daemon.pid'),
      JSON.stringify({
        pid: 999_999,
        version: '0.0.0-fake',
  ...
  it('proxy falls back to direct mode on a daemon version mismatch', async () => {
    ...
    const miniServer = net.createServer((sock) => {
      sock.write(JSON.stringify({ specship: '0.0.0-mismatch', pid: 1, socketPath: sockPath, protocol: 1 }) + '\n');
    });
  ...
  it('daemon idle-times-out after the last client disconnects', async () => {
    const env = { SPECSHIP_DAEMON_IDLE_TIMEOUT_MS: '800', SPECSHIP_PPID_POLL_MS: '200' };
    const server = spawnServer(tempDir, env);
    servers.push(server);
    sendInitialize(server.child, `file://${tempDir}`, 1);
