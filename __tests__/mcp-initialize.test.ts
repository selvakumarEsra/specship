 ...
 * the MCP server was blocking the initialize response on SpecShip.open() and
...
function spawnServer(cwd: string): ChildProcessWithoutNullStreams {
  return spawn(process.execPath, [BIN, 'serve', '--mcp'], {
    ...
    // daemon (logging to .specship/daemon.log, not the child's stderr); the
...
describe('REQ-DOMAIN-005 — no new MCP tool; instructions pointer present', () => {
  it('A3: the MCP tool list is unchanged in count and has no domain-named tool', () => {
    ...
    // + 6 designer
    // + 11 jira (specship_jira_issues — REQ-JIRA-002; specship_jira_issue —
    //   REQ-JIRA-003; specship_jira_pick — REQ-JIRA-004; specship_jira_start —
    //   REQ-JIRA-005; specship_jira_track — REQ-JIRA-008;
    //   specship_jira_coverage — REQ-JIRATEAM-004; specship_jira_reconcile —
    //   REQ-JIRATEAM-005; specship_jira_publish — REQ-JIRAPUB-001;
    //   specship_jira_transition — REQ-JIRATRANS-001; specship_jira_add_task —
    //   REQ-TASKSHIP-003; specship_jira_anchor — REQ-JIRATEAM-007) = 32.
...
describe('MCP initialize handshake (issue #172)', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-mcp-init-'));
  });
  ...
  it('responds to initialize quickly when no .specship exists in cwd', async () => {
  ...
  it('sends initialize response BEFORE tryInitializeDefault finishes', async () => {
    // Seed a real .specship so the server's tryInitializeDefault path runs
    // its full body: SpecShip.open() (which awaits initGrammars()) and then
    ...
    const json = JSON.parse(response.text);
    expect(json.id).toBe(0);
    expect(json.result.serverInfo.name).toBe('specship');
  }, 20000);
