 ...
 * Foundation Tests
 *
 * Tests for the SpecShip foundation layer.
 */
...
import { isInitialized, getSpecShipDir, validateDirectory } from '../src/directory';
import { DatabaseConnection, getDatabasePath } from '../src/db';

// Create a temporary directory for each test
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'specship-test-'));
}
...
describe('SpecShip Foundation', () => {
  ...
  describe('Initialization', () => {
    it('should initialize a new project', () => {
      ...
      expect(SpecShip.isInitialized(tempDir)).toBe(true);
      expect(fs.existsSync(getSpecShipDir(tempDir))).toBe(true);
      expect(fs.existsSync(getDatabasePath(tempDir))).toBe(true);

      cg.close();
    });

    it('should create .gitignore in .SpecShip directory', () => {
      const cg = SpecShip.initSync(tempDir);

      const gitignorePath = path.join(getSpecShipDir(tempDir), '.gitignore');
    ...
    it('should throw if already initialized', () => {
      ...
      expect(() => SpecShip.initSync(tempDir)).toThrow(/already initialized/i);
    });
  });
  ...
  describe('Opening Projects', () => {
    it('should open an existing project', () => {
      // First initialize
      const cg1 = SpecShip.initSync(tempDir);
      cg1.close();

      // Then open
      const cg2 = SpecShip.openSync(tempDir);
      expect(cg2.getProjectRoot()).toBe(path.resolve(tempDir));
      cg2.close();
    });

    it('should throw if not initialized', () => {
      expect(() => SpecShip.openSync(tempDir)).toThrow(/not initialized/i);
    });
  });

  describe('Static Methods', () => {
    it('isInitialized should return false for new directory', () => {
      expect(SpecShip.isInitialized(tempDir)).toBe(false);
    });
  ...
  describe('Uninitialize', () => {
    it('should remove .SpecShip directory', () => {
      const cg = SpecShip.initSync(tempDir);

      cg.uninitialize();

      expect(fs.existsSync(getSpecShipDir(tempDir))).toBe(false);
      expect(SpecShip.isInitialized(tempDir)).toBe(false);
    });
  });

  describe('Close/Destroy', () => {
    it('should close database but keep .SpecShip directory', () => {
      const cg = SpecShip.initSync(tempDir);
