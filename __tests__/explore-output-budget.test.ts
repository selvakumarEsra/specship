/**
 * Adaptive output budget for specship_explore (#185).
 *
 ...
 */
describe('specship_explore output respects the adaptive budget', () => {
  let testDir: string;
  let cg: SpecShip;
  let handler: ToolHandler;

  beforeAll(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-explore-budget-'));
  ...
  it('keeps total output under the small-project cap', async () => {
    const result = await handler.execute('specship_explore', { query: 'Session method helper' });
  ...
  it('prefixes source lines with line numbers by default (cat -n style)', async () => {
    delete process.env.SPECSHIP_EXPLORE_LINENUMS;
    const result = await handler.execute('specship_explore', { query: 'Session method helper' });
    const text = result.content?.[0]?.text ?? '';
  ...
  it('omits line numbers when SPECSHIP_EXPLORE_LINENUMS=0', async () => {
    process.env.SPECSHIP_EXPLORE_LINENUMS = '0';
    try {
      const result = await handler.execute('specship_explore', { query: 'Session method helper' });
      const text = result.content?.[0]?.text ?? '';
