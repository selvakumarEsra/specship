 ...
 *
 * A SpecShip index is resolved by walking up to the nearest `.specship/`.
...
describe('detectWorktreeIndexMismatch (issue #155)', () => {
  let mainRepo: string;   // main checkout — owns the .specship index
  ...
  it('warning names both trees and the fix', () => {
    ...
    expect(msg).toContain('specship init');
  });
});
...
describe('worktree mismatch surfaces on hot read tools (issue #155)', () => {
  ...
  it('prefixes a compact notice on specship_search run from a nested worktree', async () => {
    handler.setDefaultProjectHint(worktree);
    const res = await handler.execute('specship_search', { query: 'mainOnly' });
    const text = res.content[0].text;
    expect(res.isError).toBeFalsy();
    expect(text).toContain('different git worktree');
    expect(text).toContain(real(worktree));
    expect(text).toContain('specship init');
  });
  ...
  it('still shows the verbose warning on specship_status', async () => {
    handler.setDefaultProjectHint(worktree);
    const res = await handler.execute('specship_status', {});
    const text = res.content[0].text;
  ...
  it('caches detection — a later tool call needs no further git spawn', async () => {
    ...
    const first = await handler.execute('specship_search', { query: 'mainOnly' });
    ...
    process.env.PATH = '';
    try {
      const second = await handler.execute('specship_explore', { query: 'mainOnly' });
