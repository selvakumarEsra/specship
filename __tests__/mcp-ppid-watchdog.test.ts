 ...
 * propagate the death to its `specship serve --mcp` child. The child gets
 ...
 * four-tier process tree (vitest → wrapper → {stdin-holder, specship}) and
 * SIGKILL's the wrapper. The stdin-holder is a long-lived sibling whose
 * `stdout` pipe is dup'd into specship's `stdin`. After the wrapper dies
 ...
 * terminate specship then is the PPID watchdog.
 *
...
describe.skipIf(process.platform === 'win32')('MCP PPID watchdog (#277)', () => {
  ...
  it("shuts down when its parent is SIGKILL'd and stdin stays open", async () => {
    ...
    //      `stdout` pipe is dup'd into specship's `stdin`. As long as the
    //      stdin-holder is alive (it is — it's an orphan after the wrapper
    //      dies), specship's stdin never sees EOF.
    //   2. Spawns specship with that pipe as fd 0 and its stderr redirected
    //      to a tmp file that survives the wrapper, then reports both PIDs.
    //   3. Idles until SIGKILL'd from the test.
    //
    // SPECSHIP_PPID_POLL_MS=200 keeps the watchdog responsive in test; the
    // production default is 5000ms.
    const stderrLog = path.join(
      fs.mkdtempSync(path.join(os.tmpdir(), 'cg-ppid-watchdog-')),
      'specship.stderr.log',
    );
    // The wrapper waits 800ms before reporting the PIDs so the specship
    ...
    const wrapperSrc = `
      ...
      const child = spawn(process.execPath, [${JSON.stringify(BIN)}, 'serve', '--mcp'], {
        ...
        env: { ...process.env, SPECSHIP_PPID_POLL_MS: '200', SPECSHIP_NO_DAEMON: '1' },
        detached: true,
      });
      child.unref();
      setTimeout(() => {
    ...
    // specship and the stdin-holder both get reparented to init/systemd.
    // Crucially, the pipe between them stays open, so specship's stdin
    ...
    expect(
      exited,
      `specship child (pid=${childPid}) did not exit within 5s after wrapper was SIGKILL'd.\nstderr:\n${stderrContent}`,
    ).toBe(true);
