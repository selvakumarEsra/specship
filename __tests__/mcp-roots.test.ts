 ...
 * failed with a misleading "SpecShip not initialized. Run 'specship init'."
 *
...
describe('MCP project resolution via roots/list (issue #196)', () => {
  let cwdDir: string;     // where the server is launched — has NO .specship
  let projectDir: string; // the real indexed project the client reports
  let child: ChildProcessWithoutNullStreams | null = null;

  beforeEach(() => {
    cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-mcp-cwd-'));
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-mcp-proj-'));
  });
  ...
  it('resolves the project from the client roots/list when no rootUri is sent', async () => {
    const cg = await SpecShip.init(projectDir);
    cg.close();
    ...
    send(child, { jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'specship_status', arguments: {} } });
    ...
    expect(text).toContain('SpecShip Status');
    expect(text).not.toContain('No SpecShip project is loaded');
  }, 20000);
  ...
  it('returns an actionable error when there is no rootUri and no roots capability', async () => {
    ...
    expect(text).toContain('No SpecShip project is loaded');
