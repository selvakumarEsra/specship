 ...
 * Note: Git hooks functionality has been removed in favor of specship's
...
describe('Sync Module', () => {
  describe('Sync Functionality', () => {
    ...
    beforeEach(async () => {
      testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-sync-func-'));
  ...
  describe('Git-based sync', () => {
    ...
    beforeEach(async () => {
      testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-git-sync-'));
    ...
    it('should stop reporting untracked files once they are indexed (issue #206)', async () => {
      // Untracked files stay `??` in git status even after specship indexes
