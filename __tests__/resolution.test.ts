...
describe('Resolution Module', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-resolution-test-'));
  });
...
`;
  ...
  describe('Integration Tests', () => {
    it('should create resolver from SpecShip instance', async () => {
...
function processDate(input: string): string {
  ...
  return formatDate(date);
      ...
      cg = await SpecShip.init(tempDir, { index: true });
...
`
    ...
    it('TS type_alias object-shape members resolve method calls (#359)', async () => {
      ...
      fs.mkdirSync(path.join(tempDir, 'specship'));
...
`
      ...
      fs.writeFileSync(
        path.join(tempDir, 'specship', 'stdio-client.ts'),
...
#include "utils/helpers.hpp"`;
    ...
    it('should discover include directories from compile_commands.json', () => {
      ...
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-cpp-test-'));
      try {
        const compileDb = [
          {
    ...
    it('connects #include to the real header file via include-dir scan (end-to-end)', async () => {
      const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-cpp-e2e-'));
      try {
        fs.mkdirSync(path.join(tempProject, 'include'), { recursive: true });
        fs.mkdirSync(path.join(tempProject, 'src'), { recursive: true });
        fs.writeFileSync(
        ...
        clearCppIncludeDirCache();
        cg = await SpecShip.init(tempProject, { index: true });
        ...
        const db = DatabaseConnection.open(path.join(tempProject, '.specship', 'specship.db'));
