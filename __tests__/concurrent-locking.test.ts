...
describe('issue #238 — connection PRAGMAs (#1)', () => {
  ...
  beforeAll(() => {
    ...
    conn = DatabaseConnection.initialize(path.join(dir, 'specship.db'));
  });
...
describe('issue #238 — ToolHandler reuses the default instance (#2)', () => {
  ...
  it('getSpecShip(defaultRoot) returns the default instance, not a new connection', () => {
    const openSpy = vi.spyOn(SpecShip, 'openSync');
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const resolved = (handler as any).getSpecShip(root);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const nested = (handler as any).getSpecShip(path.join(root, 'does', 'not', 'exist'));
  ...
  it('concurrent read tool calls (mixed projectPath) all succeed without "database is locked"', async () => {
    ...
    try {
      const calls: Promise<{ content: Array<{ text: string }>; isError?: boolean }>[] = [
        handler.execute('specship_search', { query: 'helper' }),
        handler.execute('specship_search', { query: 'helper', projectPath: root }),
        handler.execute('specship_callers', { symbol: 'helper', projectPath: root }),
        handler.execute('specship_callees', { symbol: 'main' }),
        handler.execute('specship_files', { projectPath: root }),
        handler.execute('specship_status', { projectPath: root }),
      ];
