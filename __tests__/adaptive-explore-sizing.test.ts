/**
 * Regression test for adaptive `specship_explore` sizing — sibling
 ...
 * >= MIN_SIBLINGS (3) implementers — `specship_explore` renders it as a
 ...
 * pipeline steps stay full). Default ON; SPECSHIP_ADAPTIVE_EXPLORE=0 disables.
 *
...
describe('adaptive specship_explore sizing — sibling skeletonization', () => {
  let testDir: string;
  let cg: SpecShip;
  ...
  beforeAll(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-adaptive-explore-'));
...
`
  ...
  beforeEach(() => {
    ...
    delete process.env.SPECSHIP_ADAPTIVE_EXPLORE;
  });
  ...
  it('skeletonizes off-spine polymorphic siblings (bodies elided, signatures kept)', async () => {
    const result = await handler.execute('specship_explore', { query: QUERY, maxFiles: 12 });
  ...
  it('SPECSHIP_ADAPTIVE_EXPLORE=0 disables skeletonization (siblings render full)', async () => {
    process.env.SPECSHIP_ADAPTIVE_EXPLORE = '0';
    try {
      const result = await handler.execute('specship_explore', { query: QUERY, maxFiles: 12 });
      const text = result.content?.[0]?.text ?? '';
  ...
  it('spares an off-spine sibling when the agent NAMED a callable in it (RealCall fix)', async () => {
    const result = await handler.execute('specship_explore', { query: SPARE_QUERY, maxFiles: 15 });
    const text = result.content?.[0]?.text ?? '';
    expect(text).toContain('## Flow (call path among the symbols you queried)');
  ...
  it('naming a SHARED/polymorphic method does not spare the siblings (uniqueness-aware)', async () => {
    ...
    const result = await handler.execute('specship_explore', { query: `${QUERY} intercept`, maxFiles: 12 });
    const text = result.content?.[0]?.text ?? '';
