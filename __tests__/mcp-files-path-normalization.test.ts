/**
 * specship_files path-filter normalization (#426)
 *
...
describe('specship_files path normalization', () => {
  let tempDir: string;
  let cg: SpecShip;
  let handler: ToolHandler;

  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-files-paths-'));
  ...
  async function listed(pathFilter: string | undefined): Promise<string> {
    const result = await handler.execute('specship_files', {
