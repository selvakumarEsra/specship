/**
 * specship_explore blast-radius section.
 *
...
describe('specship_explore — blast radius', () => {
  let testDir: string;
  let cg: SpecShip;
  let handler: ToolHandler;

  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-blast-'));
  ...
  it('lists dependents (locations only) and covering tests for an entry symbol', async () => {
    const res = await handler.execute('specship_explore', { query: 'target' });
  ...
  it('omits symbols that have no dependents from the blast radius', async () => {
    const res = await handler.execute('specship_explore', { query: 'lonelyLeaf' });
    const text = res.content[0].text;
