 ...
 * `require("@specship/specship")` on their OWN Node by re-exporting the
 * compiled library that ships inside the per-platform optionalDependency
 * (@specship/specship-<target>/lib/dist/index.js).
 *
...
function installPlatformPackage(root: string, sentinel: string): void {
  const pkgRoot = path.join(root, 'node_modules', '@specship', `specship-${target}`);
  writeFakeLib(path.join(pkgRoot, 'lib', 'dist'), sentinel);
  fs.writeFileSync(
    path.join(pkgRoot, 'package.json'),
    JSON.stringify({ name: `@specship/specship-${target}`, version: VERSION }) + '\n'
  );
}
