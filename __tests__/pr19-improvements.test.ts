...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'specship-pr19-test-'));
}
...
// =============================================================================
// Graph Traversal 'both' Direction Fix
// (requires better-sqlite3 - will use SpecShip integration)
// =============================================================================
...
describe('Graph Traversal Both Direction', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should traverse both directions from a node', async () => {
    const SpecShip = (await import('../src/index')).default;
...
`);
    ...
    const cg = SpecShip.initSync(testDir, {
...
describe('Database Layer Improvements', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should support batch insert of unresolved refs', async () => {
    ...
    const dbPath = path.join(testDir, 'specship.db');
...
`);
      ...
      const cg = SpecShip.initSync(tmpDir, {
        config: { include: ['src/**/*.ts'], exclude: [] },
      });
      await cg.indexAll();
...
`);
      ...
      // note) so specship_node can hand back every overload and the agent never
...
describe('CLI uninit', () => {
  ...
  it.skipIf(!HAS_SQLITE)('should uninitialize a project via SpecShip.uninitialize()', async () => {
    const SpecShip = (await import('../src/index')).default;

    // Initialize
    const cg = SpecShip.initSync(testDir);
    expect(SpecShip.isInitialized(testDir)).toBe(true);

    // Uninitialize
    cg.uninitialize();

    // .specship directory should be removed
    expect(SpecShip.isInitialized(testDir)).toBe(false);
  });
});
