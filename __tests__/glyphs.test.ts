...
describe('supportsUnicode', () => {
  ...
  it('returns false on Windows by default (mojibake-prone consoles)', () => {
    withEnv({ SPECSHIP_ASCII: undefined, SPECSHIP_UNICODE: undefined, TERM: undefined }, () => {
      setPlatform('win32');
  ...
  it('returns false on Linux kernel console (TERM=linux)', () => {
    withEnv({ SPECSHIP_ASCII: undefined, SPECSHIP_UNICODE: undefined, TERM: 'linux' }, () => {
      setPlatform('linux');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('respects SPECSHIP_UNICODE=1 on Windows (opt-in escape hatch)', () => {
    withEnv({ SPECSHIP_UNICODE: '1', SPECSHIP_ASCII: undefined }, () => {
      setPlatform('win32');
      expect(supportsUnicode()).toBe(true);
    });
  });

  it('respects SPECSHIP_ASCII=1 on macOS (opt-out escape hatch)', () => {
    withEnv({ SPECSHIP_ASCII: '1', SPECSHIP_UNICODE: undefined }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });

  it('SPECSHIP_ASCII takes precedence over SPECSHIP_UNICODE', () => {
    withEnv({ SPECSHIP_ASCII: '1', SPECSHIP_UNICODE: '1' }, () => {
      setPlatform('darwin');
      expect(supportsUnicode()).toBe(false);
    });
  });
...
describe('getGlyphs', () => {
  ...
  it('returns ASCII glyphs on Windows', () => {
    withEnv({ SPECSHIP_ASCII: undefined, SPECSHIP_UNICODE: undefined }, () => {
      setPlatform('win32');
      const g = getGlyphs();
