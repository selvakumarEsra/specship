...
describe('git sync hooks', () => {
  ...
  beforeEach(() => {
    repo = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-githooks-'));
  });
  ...
  it('installs all default hooks, executable, invoking specship sync', () => {
    gitInit(repo);
    ...
    for (const hook of DEFAULT_SYNC_HOOKS) {
      ...
      expect(body).toContain('specship sync');
      expect(body).toContain('command -v specship'); // no-op when not on PATH
  ...
  it('is idempotent — re-install does not duplicate the block', () => {
    ...
    const occurrences = body.split('# >>> specship sync hook >>>').length - 1;
  ...
  it('remove keeps user content when the hook is shared', () => {
    ...
    expect(body).not.toContain('specship sync');
  });
