...
import { SpecShip } from '../../src/index.js';
...
if (!fs.existsSync(path.join(resolvedPath, '.specship', 'specship.db'))) {
  console.error(`No .specship/specship.db found at ${resolvedPath}`);
  process.exit(1);
}

let specshipSha = 'unknown';
try {
  specshipSha = execSync('git rev-parse --short HEAD', { encoding: 'utf-8' }).trim();
} catch {}

console.log(`\nSpecShip Eval — ${path.basename(resolvedPath)}`);
console.log(`Codebase: ${resolvedPath}`);
console.log(`Commit:   ${specshipSha}`);
console.log(`Cases:    ${testCases.length}`);
console.log('');

async function run() {
  const cg = SpecShip.openSync(resolvedPath);
