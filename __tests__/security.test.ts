...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'specship-security-test-'));
}
...
describe('MCP Input Validation', () => {
  ...
  it('should reject non-string query in specship_search', async () => {
    const result = await handler.execute('specship_search', { query: null });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should reject empty string query in specship_search', async () => {
    const result = await handler.execute('specship_search', { query: '' });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should accept valid query in specship_search', async () => {
    const result = await handler.execute('specship_search', { query: 'example' });
    expect(result.isError).toBeFalsy();
  });

  it('should clamp limit to valid range in specship_search', async () => {
    // Extremely large limit should still work (clamped to 100)
    const result = await handler.execute('specship_search', { query: 'example', limit: 999999 });
    expect(result.isError).toBeFalsy();
  });

  it('should reject non-string symbol in specship_callers', async () => {
    const result = await handler.execute('specship_callers', { symbol: 123 });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should reject non-string query in specship_explore', async () => {
    const result = await handler.execute('specship_explore', { query: undefined });
    expect(result.isError).toBe(true);
    expect(result.content[0].text).toContain('non-empty string');
  });

  it('should truncate oversized tool output', async () => {
    // Force a huge result set through specship_search; the response must be
    ...
    const fakeHandler = new ToolHandler(fakeCg as unknown as SpecShip);

    const result = await fakeHandler.execute('specship_search', { query: 'x' });

    expect(result.isError).toBeFalsy();
    expect(result.content[0].text).toContain('... (output truncated)');
  });

  it('should reject non-string symbol in specship_impact', async () => {
    const result = await handler.execute('specship_impact', { symbol: [] });
    expect(result.isError).toBe(true);
  });

  it('should reject non-string symbol in specship_node', async () => {
    const result = await handler.execute('specship_node', { symbol: false });
    expect(result.isError).toBe(true);
  });

  it('should reject non-string symbol in specship_callees', async () => {
    const result = await handler.execute('specship_callees', { symbol: {} });
    expect(result.isError).toBe(true);
  });

  it('should handle NaN limit gracefully', async () => {
    const result = await handler.execute('specship_search', { query: 'example', limit: 'abc' });
    expect(result.isError).toBeFalsy();
  });

  it('should handle negative limit gracefully', async () => {
    const result = await handler.execute('specship_search', { query: 'example', limit: -5 });
    expect(result.isError).toBeFalsy();
  });

  // #230: getSpecShip must reject a sensitive system directory passed as
