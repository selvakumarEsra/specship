 ...
 *
 * Pinned because the lookup vocabulary is what makes specship useful
...
function tmpRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'specship-symbol-lookup-'));
}
...
describe.skipIf(!HAS_SQLITE)('matchesSymbol — module-qualified lookups (#173)', () => {
  ...
  beforeEach(async () => {
    ...
    cg = SpecShip.initSync(projectRoot, {
  ...
  it('specship_node with a `file` hint pins an overloaded name to that file', async () => {
    ...
    const res = await handler.execute('specship_node', {
      symbol: 'run',
      includeCode: true,
...
describe.skipIf(!HAS_SQLITE)('matchesSymbol — dotted lookups (regression for #173 fix)', () => {
  ...
  it('specship_node on an ambiguous bare name returns ALL overloads with bodies (no guess)', async () => {
    ...
    const res = await handler.execute('specship_node', { symbol: 'request', includeCode: true });
    const text = res.content?.[0]?.text ?? '';
