 ...
 * (`Fatal process out of memory: Zone`) that crashed `specship index` on large
...
describe('buildRelaunchArgv', () => {
  it('places the wasm flags first, then the script and its args', () => {
    expect(buildRelaunchArgv('/x/specship.js', ['index', '/repo'], [])).toEqual([
      '--liftoff-only',
      '/x/specship.js',
      'index',
      '/repo',
  ...
  it('preserves other existing node flags without duplicating ours', () => {
    expect(
      buildRelaunchArgv('/x/specship.js', ['status'], ['--liftoff-only', '--enable-source-maps'])
    ).toEqual(['--liftoff-only', '--enable-source-maps', '/x/specship.js', 'status']);
  });
