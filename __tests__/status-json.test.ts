/**
 * Tests for the CI/scripting fields `specship status --json` exposes (issue
 * #329): the `version`, `indexPath`, and `lastIndexed` fields, plus the
 * matching `SpecShip.getLastIndexedAt()` library method.
 *
...
describe('specship status --json — CI fields (#329)', () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-status-json-'));
  });
  afterEach(() => {
  ...
  it('status --json on an UNINITIALIZED project reports version + indexPath + lastIndexed:null', () => {
    ...
    expect(out.indexPath as string).toContain('.specship');
