 ...
 * the full real path: real SpecShip index + real ToolHandler.execute().
 *
...
describe('MCP staleness banner', () => {
  ...
  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-stale-banner-'));
    ...
    // files shared imports/calls, specship_search responses would surface
  ...
  it('prepends a stale banner when the response references a pending file', async () => {
    ...
    const res = await handler.execute('specship_search', { query: 'alphaOnly' });
  ...
  it('lists pending files under "Pending sync" in specship_status', async () => {
    cg.watch({ debounceMs: 4000, inertForTests: true });
    await cg.waitUntilWatcherReady();
