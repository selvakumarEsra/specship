...
describe('FileWatcher', () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-watcher-'));
  ...
  describe('filtering', () => {
    ...
    it('should ignore .specship directory changes', async () => {
      const syncFn = vi.fn().mockResolvedValue({ filesChanged: 0, durationMs: 0 });
      const watcher = newWatcher(syncFn, { debounceMs: 200 });
      ...
      // A .specship event — FileWatcher's `isAlwaysIgnored` filter must drop
      // it before scheduling sync.
      __emitWatchEventForTests(testDir, '.specship/db.sqlite');
  ...
  describe('pending file tracking (#403)', () => {
    ...
    it('should retain pending files and retry when syncFn throws LockUnavailableError (#449)', async () => {
      // SpecShip.watch() converts the cross-process lock-failure no-op
  ...
  describe('SpecShip integration', () => {
    ...
    it('should watch and unwatch via SpecShip API', async () => {
