...
describe('MCP catch-up gate', () => {
  ...
  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-catchup-gate-'));
  ...
  it('awaits the gate before serving the first tool call', async () => {
    ...
    const res = await handler.execute('specship_search', { query: 'survivor' });
  ...
  it('drops the gate after first await — second call does not re-wait', async () => {
    ...
    await handler.execute('specship_search', { query: 'survivor' });
  ...
  it('catch-up reconciles a deleted file before the first tool call sees it', async () => {
    ...
    const res = await handler.execute('specship_search', { query: 'deletedLater' });
    expect(res.isError).toBeFalsy();
