 ...
 * SPECSHIP_DOWNLOAD_BASE — no real network, no published release needed. The
...
const asset = `specship-${target}.tar.gz`;
...
// A temp dir standing in for the installed @selvakumaresra/specship main package.
function makePkg(version = '9.9.9-test'): string {
  const dir = mkTmp('pkg');
  fs.copyFileSync(SHIM_SRC, path.join(dir, 'npm-shim.js'));
  fs.writeFileSync(path.join(dir, 'package.json'),
    JSON.stringify({ name: '@selvakumaresra/specship', version }) + '\n');
  return dir;
}
...
function writeLauncher(binDir: string): void {
  ...
  const p = path.join(binDir, 'specship');
...
describe.skipIf(isWindows)('npm-shim launcher', () => {
  it('runs the installed optional-dependency bundle without any download', async () => {
    const pkg = makePkg();
    const platformPkg = path.join(pkg, 'node_modules', '@selvakumaresra', `specship-${target}`);
    writeLauncher(path.join(platformPkg, 'bin'));
    fs.writeFileSync(path.join(platformPkg, 'package.json'),
      JSON.stringify({ name: `@selvakumaresra/specship-${target}`, version: '9.9.9-test' }) + '\n');
    const cache = mkTmp('cache');
    const r = await runShim(pkg, ['--probe-abc'], { SPECSHIP_INSTALL_DIR: cache });
  ...
  it('uses an already-cached bundle even when downloads are disabled', async () => {
    ...
    const r = await runShim(pkg, ['--probe-xyz'], {
      SPECSHIP_INSTALL_DIR: cache,
      SPECSHIP_NO_DOWNLOAD: '1',
    });
  ...
  it('prints actionable guidance and exits 1 when disabled with no bundle', async () => {
    ...
    const r = await runShim(pkg, ['--version'], {
      SPECSHIP_INSTALL_DIR: mkTmp('cache'),
      SPECSHIP_NO_DOWNLOAD: '1',
    });

    expect(r.status).toBe(1);
    expect(r.stderr).toContain(`no prebuilt bundle for ${target}`);
    expect(r.stderr).toContain(`@selvakumaresra/specship-${target}`);
...
describe.skipIf(!CAN_NET)('npm-shim download fallback (local HTTPS)', () => {
  ...
  beforeAll(async () => {
    ...
    // Build a fake bundle archive (specship-<target>/bin/specship), like a real release asset.
    const work = mkTmp('fixture');
    writeLauncher(path.join(work, `specship-${target}`, 'bin'));
    const archive = path.join(work, asset);
    execSync(`tar -czf ${JSON.stringify(archive)} -C ${JSON.stringify(work)} specship-${target}`);
  ...
  function netEnv(cache: string): Record<string, string> {
    return {
      SPECSHIP_INSTALL_DIR: cache,
      SPECSHIP_DOWNLOAD_BASE: `https://127.0.0.1:${port}`,
  ...
  it('downloads, verifies the checksum, extracts, and execs the bundle', async () => {
    ...
    expect(fs.existsSync(path.join(cache, 'bundles', `${target}-5.0.0-net`, 'bin', 'specship'))).toBe(true);
  }, 20000);
