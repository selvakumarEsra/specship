...
describe('watchDisabledReason', () => {
  it('returns a reason when SPECSHIP_NO_WATCH=1', () => {
    const reason = watchDisabledReason('/home/me/project', {
      env: { SPECSHIP_NO_WATCH: '1' },
      isWsl: false,
    });
    expect(reason).toBeTruthy();
    expect(reason).toMatch(/SPECSHIP_NO_WATCH/);
  });
  ...
  it('SPECSHIP_FORCE_WATCH=1 overrides WSL auto-detect', () => {
    const reason = watchDisabledReason('/mnt/d/code/project', {
      env: { SPECSHIP_FORCE_WATCH: '1' },
      isWsl: true,
    });
    expect(reason).toBeNull();
  });

  it('SPECSHIP_NO_WATCH wins over SPECSHIP_FORCE_WATCH', () => {
    const reason = watchDisabledReason('/home/me/project', {
      env: { SPECSHIP_NO_WATCH: '1', SPECSHIP_FORCE_WATCH: '1' },
      isWsl: false,
    });
    expect(reason).toBeTruthy();
  });
...
describe('FileWatcher honors the watch policy', () => {
  ...
  afterEach(() => {
    delete process.env.SPECSHIP_NO_WATCH;
    if (testDir && fs.existsSync(testDir)) {
      fs.rmSync(testDir, { recursive: true, force: true });
    }
  });

  it('does not start when SPECSHIP_NO_WATCH=1', () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specship-nowatch-'));
