...
SpecShip turns source code into a queryable graph in four stages.
...
Everything goes into a local SQLite database (`.specship/specship.db`) with FTS5 full-text search. SpecShip uses native `better-sqlite3` when available and transparently falls back to a WASM backend; `specship status` shows which is live.
