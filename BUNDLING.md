...
SpecShip ships a **vendored Node runtime** alongside the app. Because Node 22.5+
...
specship-<target>/
  ...
  bin/
    specship | specship.cmd   # launcher → runs the bundled Node with the app
```
...
2. **npm** ([`scripts/npm-shim.js`](scripts/npm-shim.js)) — preserves
   `npm i -g @specship/specship@0.5.0`. The main package is a tiny shim; the
...
- Retire the now-vestigial Node-version gate in `src/bin/specship.ts` — the
