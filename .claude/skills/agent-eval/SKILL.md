---
name: agent-eval
description: Benchmark SpecShip retrieval quality on a real codebase by comparing agent behavior with vs without SpecShip. Use when the user runs /agent-eval or asks to test, benchmark, audit, or validate a specship version (the local dev build or a published npm version) against a language's repo.
---

# SpecShip Quality Audit

Measures how much SpecShip helps an agent versus plain grep/read, for a chosen
specship version on a chosen real-world repo. Drives the harness in
`scripts/agent-eval/`.
...
**Step 1 — version.** Ask with `AskUserQuestion`: which specship version to test.
...
- Headless (`parse-run.mjs`): total tool calls, file `Read`s, Grep/Bash,
  specship-tool calls, duration, **total cost**.
- Interactive (`parse-session.mjs`): the `VERDICT: specship_explore used Nx |
...
specship reduced effort and whether both arms reached a correct answer.

## Notes
- The index is rebuilt every run (`audit.sh` wipes `.specship`) — different
  versions extract differently, so an index must be served by the same binary
  that built it.
- `audit.sh` temporarily mutates the global `specship` install for the test,
  then restores your dev link via `local-install.sh`.
- Corpus repos are cloned to `/tmp/specship-corpus` (reused if already present).
