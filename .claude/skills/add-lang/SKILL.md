---
name: add-lang
description: Add tree-sitter language support to specship end-to-end — wire the grammar + extractor, write tests, then benchmark extraction quality and retrieval value on 3 popular real-world repos. Use when the user runs /add-lang <language> or asks to add/support a new language (e.g. Lua, Elixir, Zig, OCaml) in specship.
---

# Add a language to SpecShip

Wire a new tree-sitter language into specship's extraction pipeline, prove it
extracts real symbols on popular repos, and prove it beats no-specship for an
...
## Prerequisites
- Run from the specship repo root. `node`, `git`, `gh`, and a logged-in
...
1. **`src/types.ts`** — TWO edits:
   ...
   - add `'**/*.<ext>',` to `DEFAULT_CONFIG.include`. **Don't skip this** — it's
     the file-scan allowlist; without the glob, `specship init` finds **0
...
```bash
( cd <sample-repo> && specship init -i )
...
Make the dev build the specship on PATH **once**, then loop:
```bash
npm run build && ./scripts/local-install.sh
scripts/add-lang/bench.sh <lang> <name> <url> "<question>" headless   # ×3
```
`bench.sh` clones (shared `/tmp/specship-corpus`), wipes + indexes, runs
...
`Read`s, Grep/Bash, specship-tool calls, duration, and **cost** — for both the
...
- **CHANGELOG.md**: add an `## [Unreleased]` section at the top (above the
  ...
  *"SpecShip now indexes **<Lang>** (`.ext`) — functions, classes, imports, and
...
- **A/B** per repo: `with` vs `without` (tool calls, file Reads, cost) and a
  one-line verdict — did specship reduce effort, and did both arms reach a
  correct answer?
...
- The A/B spawns real **paid** `claude -p` runs (opus, `--max-budget-usd`),
  2 arms × 3 repos. The corpus dir `/tmp/specship-corpus` is shared with
