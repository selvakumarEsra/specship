...
summary: Investigated why specship's read savings don't convert to wall-clock; root cause is agent tool-CHOICE (under-uses trace). Shipped a chain of fixes; the breakthrough is "explore-surfaces-flow" — the first mechanism to show up in real agent runs by adapting the tool the agent already uses.
---

# Handoff: specship retrieval — tool adoption & explore-surfaces-flow

## Resume here — read this first
**Current state:** A long investigation into making agents answer flow questions faster with specship. 6 commits on `architectural-improvements` (all probe-validated, suite green 815). The breakthrough: **`specship_explore` now surfaces the execution flow** from the symbol-bag the agent already passes it (`PmsProductController getList PmsProductService list PmsProductServiceImpl` → leads output with `getList → service-interface → impl`, riding synth edges). It's the FIRST mechanism this whole arc to actually appear in real agent runs (spring-mall A/B: flow surfaced both runs, reads 2.0→1.5) — because it adapts the tool the agent USES instead of trying to make it use `trace`.
...
## Goal
Make an AI agent answer **flow questions** ("how does X reach Y", request→handler→service, state→render) fast: ~0 Read/Grep, few specship calls, lower wall-clock. `specship_trace` is the fastest tool (1 call = the path), but the agent under-uses it. Ultimate target = trace's speed, however the agent gets there.

## Key findings (the through-line)
- **The wall is agent tool-CHOICE, not the graph.** Matrix-wide, specship cuts reads −75% but wall-clock only −16% (`docs/benchmarks/specship-ab-matrix.md`). The floor is round-trips + the synthesis turn. The agent reliably calls `context`/`explore`, rarely `trace` (3/37 flow cells). Full analysis: `docs/benchmarks/call-sequence-analysis.md`.
...
- **Resolver/synthesizer changes need a CLEAN reindex**: `rm -rf .specship && specship init -i` (the init edge count is contains-only — query the DB for the real count). The explore-flow change is query-time (no reindex).
...
- Synthesizer: `sqlite3 <repo>/.specship/specship.db "select count(*) from edges where json_extract(metadata,'$.synthesizedBy')='interface-impl'"`; node count stable before/after reindex (synth adds edges only).
...
## Repo state
- branch `architectural-improvements`, last commit `bafae81 feat(mcp): specship_explore surfaces the execution flow from its named symbols`.
- uncommitted: clean (only untracked `.claude/handoffs/`).
- 6 session commits: `eab5cf3` self-sufficient trace + `SPECSHIP_MCP_TOOLS` allowlist · `a6183d7` research log + arms harness · `bde8c19` node/trace line numbers · `98baf41` Java/Kotlin interface→impl synthesizer · `6f3c468` playbook · `bafae81` explore-surfaces-flow.
...
- Ablation arms A–E (`run-arms.sh`/`arms-F.sh` + `SPECSHIP_MCP_TOOLS` allowlist). explore = 68% of payload, load-bearing; trace path-scoped but under-adopted; trace alone insufficient.
