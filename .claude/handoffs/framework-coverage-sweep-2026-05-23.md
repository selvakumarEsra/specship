...
## Goal
Close static-extraction holes for **dynamic dispatch** across every language/framework specship supports, so cross-symbol flows (request→route→handler→service, state→render, virtual→override) exist in the graph and an agent answers flow questions with few specship calls and ~0 Read/Grep. Per framework/language: canonical flow `trace`s end-to-end, agent A/B shows fewer reads, no node explosion, recorded in `docs/design/dynamic-dispatch-coverage-playbook.md` (the matrix §6 + per-item notes §7). **This goal is now met; what remains is ship-prep + documented frontiers.**
...
- **Reindex picks up resolver changes only on a CLEAN index:** `specship index` is incremental (skips unchanged files); after `npm run build`, do `rm -rf .specship && specship init -i` to re-extract. The init message's edge count is contains-only (~misleading); query the DB for the real count.
...
- Global `specship` is npm-linked to this repo's `dist/`; `npm run build` then reindex. A/B harness: `scripts/agent-eval/run-all.sh <repo> "<Q>" headless` (with vs empty MCP), parse via `node scripts/agent-eval/parse-run.mjs`.

## How to test & validate (the per-framework loop)
- Corpus in `/tmp/specship-corpus/<name>` (clone S/M/L, `git clone --depth 1`). Index: `rm -rf .specship && specship init -i`.
- Measure holes: `sqlite3 .specship/specship.db "select count(*) from nodes where kind='route'"` + route→handler edges (`join edges on source where kind='references'`). Node-count before/after (no explosion).
- Flow: `node scripts/agent-eval/probe-node.mjs <repo> <symbol>` (shows Called-by/Calls trail) / `probe-trace.mjs <repo> <from> <to>`.
- Agent A/B (≥2 runs/arm, variance is real): `run-all.sh` headless, record Read/Grep/duration/specship. Pass = fewer reads with specship.
- Tests: `npm test` (vitest). Resolver extract tests in `__tests__/frameworks.test.ts`; end-to-end in `__tests__/frameworks-integration.test.ts` (real SpecShip + indexAll); Dart range in `__tests__/extraction.test.ts`; Drupal in `__tests__/drupal.test.ts`.
