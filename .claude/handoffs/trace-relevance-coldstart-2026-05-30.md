...
- `5d7388c` Skeleton/focused tag steers to `specship_explore`, not Read (spiral fix #1).
- `dc19eab` Bench parser race-aware (excludes "No such tool available" runs).
- `91e28df` serve --mcp cold-start ~811ms→~600ms (defer SpecShip load + 25ms poll).
...
- **Kill stale daemons before any cold-start measurement:** `pkill -9 -f "dist/bin/specship.js"; rm -f /tmp/specship-corpus/<repo>/.specship/daemon.*`. A zombie daemon holding the lock causes a 6s retry-exhaust that looks like a 7× regression (it bit me — the "6239ms" false alarm).
...
- Corpus repos: `/tmp/specship-corpus/<repo>` (all 7 README repos indexed). Explore/trace changes are **query-time** (no re-index). The closure-collection synthesizer is **index-time** but produces 0 edges on non-Swift, so it's inert there.
- Global `specship` is npm-linked to the dev dist (`node dist/bin/specship.js`). **Always `npm run build` before any probe/A/B** (they load `dist/`, not `src/`).
- `engine.ts`/`tools.ts` now `import type SpecShip` + lazy `require('../index')` (CommonJS, cached) so the daemon binds before the sqlite/query chain loads; `findNearestSpecShipRoot` now comes from the light `../directory`.
...
- Cold-start handshake (after killing daemons): node-spawn a `serve --mcp`, send `initialize`, time the id:1 response → **~90ms** (was ~811ms). Then a `tools/call` (e.g. `specship_status`) returns a real result (forwarded to the daemon, ~3.4s on vscode's first index load — a call that returns LATE, not a missing-tool error).
...
- **Outcome:** Synthesizer correct + sufficient (forced specship-only 3/3) but didn't move the noisy median.
...
- **Claude:** Bench parser race-aware (`dc19eab`). Deferred SpecShip load off the bind path + 25ms poll (`91e28df`, 811→600ms).
