...
- **Always `npm run build` before probing/A/B** — probes + the A/B MCP server load `dist/`, not `src/`. Corpus indexes (`/tmp/specship-corpus/*`) are valid without re-index since all changes are query-time.
...
## Repo state
- branch `main`, last commit `b026e64 feat(mcp): per-symbol adaptive specship_explore sizing (#569)`.
