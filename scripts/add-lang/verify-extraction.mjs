#!/usr/bin/env node
// Sanity-check that specship extracted REAL symbols (not just file/import nodes)
...
// Reads `specship status <repo> --json` using whatever specship is on PATH,
...
let status;
try {
  const out = execFileSync('specship', ['status', repo, '--json'], { encoding: 'utf8' });
  status = JSON.parse(out);
} catch (e) {
  console.error(`[verify] could not read specship status for ${repo}: ${e.message}`);
  process.exit(2);
}

// Kinds that prove the extractor mapped AST node types (everything except
// 'file' and 'import', which specship creates structurally for any language).
