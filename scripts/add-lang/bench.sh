...
#   clone -> wipe+index (with the specship on PATH) -> verify extraction ->
#   with/without retrieval A/B (reuses scripts/agent-eval/run-all.sh).
#
# Assumes the specship dev build is already built + linked on PATH — the skill
...
# Env:   CORPUS   corpus dir (default /tmp/specship-corpus, shared with agent-eval)
set -uo pipefail
...
command -v specship >/dev/null || { echo "no specship on PATH (build + ./scripts/local-install.sh first)"; exit 1; }

echo "==================== add-lang bench: $NAME ($LANG_TOKEN) ===================="
echo "specship: $(command -v specship) -> $(specship --version 2>/dev/null || echo '?')"
...
echo "→ wiping .specship and indexing"
rm -rf "$REPO/.specship"
( cd "$REPO" && specship init -i ) || { echo "indexing failed"; exit 1; }
