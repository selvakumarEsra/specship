#!/usr/bin/env bash
# One-shot SpecShip quality audit:
...
# Env: CORPUS  corpus dir (default: /tmp/specship-corpus)
set -uo pipefail
...
# 1. Set the specship version under test (mutates the global install).
...
ACTUAL="$(specship --version 2>/dev/null || echo '?')"
echo "  specship on PATH: $(command -v specship) -> $ACTUAL"
...
echo "→ [3/4] wiping .specship and re-indexing with $ACTUAL"
rm -rf "$REPO/.specship"
...
if ( cd "$REPO_ROOT" && ./scripts/local-install.sh >/dev/null 2>&1 ); then
  echo "  global specship restored to dev build"
else
