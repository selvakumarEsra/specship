#!/usr/bin/env bash
# Re-run the README "Benchmark Results" A/B (with vs without specship) on the
...
# $CORPUS (default /tmp/specship-corpus) by the build under test.
set -uo pipefail
H="$(cd "$(dirname "$0")" && pwd)"
C="${CORPUS:-/tmp/specship-corpus}"
RUNS="${RUNS:-4}"
ROWS=(
