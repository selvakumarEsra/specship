...
- **SpecShip now learns from how you work — and proposes durable fixes you can apply in one click.** A new reflection engine mines your ingested Claude Code transcripts for recurring, fixable patterns — re-reading the same file instead of querying the graph, reaching for grep over `specship_search`, repeating the same ask, running the same command by hand, using destructive shell commands (`rm -rf`, `git push --force`, `git reset --hard`), repeatedly editing the same hotspot file, giving the same correction over and over, reading and grepping heavily without ever querying SpecShip, dumping a huge command's output into context, or re-reading the same doc across many sessions — and turns each into a concrete, evidence-backed proposal: a rule for your project `CLAUDE.md`, a portable note in `~/.claude/memory`, a new `commands/ss-*` slash command, or a `.claude/settings.json` hook. Nothing is ever written without you — each proposal shows an exact preview diff, and applying it is reversible (an Undo strips precisely what was added) and idempotent. Review them on the new **Improvements** page in the dashboard, or run `specship reflect` in your terminal. A daily background sweep raises a desktop notification only when a new high-severity proposal appears (toggle it under **Settings → Notifications**), and you can dismiss any proposal so it stops resurfacing.
