...
- New `specship memory capture` command records a lesson or anti-pattern as a reviewable memory rule — targeting a portable `~/.claude` memory note or your project `CLAUDE.md` — so a mistake you don't want repeated gets loaded into the next session. Human-gated like every reflection proposal: nothing is written until you review the exact change and apply it.
