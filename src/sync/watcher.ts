 ...
 * log once — partial live-watch (with `specship sync` as the backstop) is far
 * better than exhausting the user's inotify budget and breaking watching
 * system-wide (#579). Tunable via SPECSHIP_MAX_DIR_WATCHES.
 */
const DEFAULT_MAX_DIR_WATCHES = 50_000;

function maxDirWatches(): number {
  const raw = process.env.SPECSHIP_MAX_DIR_WATCHES;
  if (raw && /^\d+$/.test(raw)) {
    const n = Number(raw);
    if (n > 0) return n;
  }
...
export class LockUnavailableError extends Error {
  constructor(message = 'SpecShip file lock unavailable; another process is writing') {
    super(message);
...
export class FileWatcher {
  ...
  start(): boolean {
    ...
    // there; callers fall back to manual `specship sync` or git sync hooks.
  ...
  private isAlwaysIgnored(rel: string): boolean {
    return (
      rel === '.specship' || rel.startsWith('.specship/') ||
