 ...
 *    canonical `hook_X` name, linking implementations to the hook when `specship_callers`
 *    is invoked.
 *
 ...
 *   `specship_search("form_alter")`. Full hook-node creation (virtual nodes for every hook)
 ...
 * - TODO: Improve hook resolution: create virtual `hook_*` nodes so `specship_callers`
