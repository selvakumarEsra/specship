 ...
 *
 * There is no `writeClaudeMd` shim anymore: specship stopped writing a
...
export function hasMcpConfig(location: InstallLocation): boolean {
  ...
  return !!config.mcpServers?.specship;
}
...
export function hasPermissions(location: InstallLocation): boolean {
  ...
  return allow.some((p: string) => p.startsWith('mcp__specship__'));
}
