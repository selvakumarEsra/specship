...
export const designerToolDefinitions: ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Promote: fetch the project's export zip and extract it under ./artifacts/<key>/handoff-<ts>/project/, plus decision-record.md (the live chat transcript, verbatim). Call when the human says 'yes, that's it'. The bundle is what /specship:spec design feeds to the claude-design-implement workflow.",
    inputSchema: {
      type: 'object',
      properties: {
        key: { type: 'string', description: 'Stable key for this loop. Defaults to "default".' },
