 ...
 *
 * One shared `specship serve --mcp` daemon per project root means we need a
 ...
 *
 * Both live under `.specship/` so the project-scoped uninstall (`specship
...
export function getDaemonSocketPath(projectRoot: string): string {
  if (process.platform === 'win32') {
    return `\\\\.\\pipe\\specship-${projectHash(projectRoot)}`;
  }
  const inProject = path.join(getSpecShipDir(projectRoot), 'daemon.sock');
  ...
  return path.join(os.tmpdir(), `specship-${projectHash(projectRoot)}.sock`);
}

/** Absolute path to the daemon pid lockfile for `projectRoot`. */
export function getDaemonPidPath(projectRoot: string): string {
  return path.join(getSpecShipDir(projectRoot), 'daemon.pid');
}
