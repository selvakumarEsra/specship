...
- **"Generate / refresh the regression pack" / "build regression tests in JIRA"** → \`specship_jira_regression_pack\` — creates or refreshes the SpecShip Regression Pack epic in the bound project, with one domain-area Story per area (\`Uncategorised\` until REQ-JIRAREG-002 lands) and one Sub-task per acceptance criterion of every implemented (or verified) requirement. Idempotent: a re-run with no spec changes performs zero JIRA writes. Pass \`dry_run: true\` to preview the plan without touching JIRA.
