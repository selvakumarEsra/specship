 ...
 *
 * One detached `specship serve --mcp` daemon process per project root,
 ...
 *     `SPECSHIP_DAEMON_IDLE_TIMEOUT_MS` (default 300s) so back-to-back agent
 ...
 *   - The lockfile (`.specship/daemon.pid`) competing daemons arbitrate
...
export interface DaemonHello {
  specship: string; // package version (must match the proxy's own version)
...
export class Daemon {
  ...
  async start(): Promise<DaemonStartResult> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      server.listen(this.socketPath, () => {
        ...
        // `.specship/`, which is git-ignored but may be on a shared FS.
        if (process.platform !== 'win32') {
    ...
    process.stderr.write(
      `[SpecShip daemon] Listening on ${this.socketPath} (pid ${process.pid}, v${SpecShipPackageVersion}). Idle timeout ${this.idleTimeoutMs}ms.\n`
    );
  ...
  async stop(reason: string = 'stop'): Promise<void> {
    ...
    process.stderr.write(`[SpecShip daemon] Shutting down (${reason}; clients=${this.clients.size}).\n`);
  ...
  private handleConnection(socket: net.Socket): void {
    ...
    const hello: DaemonHello = {
      specship: SpecShipPackageVersion,
      pid: process.pid,
      socketPath: this.socketPath,
      protocol: 1,
    };
...
export function tryAcquireDaemonLock(projectRoot: string): AcquireResult {
  ...
  // Make sure the .specship/ directory exists — the daemon may be the first
...
function resolveIdleTimeoutMs(): number {
  const raw = process.env.SPECSHIP_DAEMON_IDLE_TIMEOUT_MS;
