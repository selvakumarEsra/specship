 ...
 * SpecShip MCP Server
 *
 * Model Context Protocol server that exposes SpecShip functionality
 ...
 * @example
 * ```typescript
 * import { MCPServer } from 'specship';
 *
 ...
 *   behavior; used when the user opts out (`SPECSHIP_NO_DAEMON=1`), no
 *   `.specship/` is reachable, or the daemon machinery fails for any reason.
 ...
 *   sharing one SpecShip + watcher + SQLite handle. Spawned on demand; never a
...
import { findNearestSpecShipRoot, getSpecShipDir } from '../directory';
 ...
 */
const DAEMON_INTERNAL_ENV = 'SPECSHIP_DAEMON_INTERNAL';
...
// are cheap. Paired with deferring the SpecShip load (engine.ts) off the bind
...
/** Whether `SPECSHIP_NO_DAEMON` was set to a truthy value. */
function daemonOptOutSet(): boolean {
  const raw = process.env.SPECSHIP_NO_DAEMON;
  if (!raw) return false;
 ...
 * `null` when no `.specship/` is reachable from the candidate path — in
 * that case the caller must run in direct mode, since the daemon lockfile
 * and socket both live under `.specship/`.
 *
...
function resolveDaemonRoot(explicitPath: string | null): string | null {
  ...
  const root = findNearestSpecShipRoot(candidate);
  if (!root) return null;
 ...
 * `.specship/daemon.log`). Re-invokes the *same* CLI faithfully across dev and
...
function spawnDetachedDaemon(root: string): void {
  ...
  try {
    logFd = fs.openSync(path.join(getSpecShipDir(root), 'daemon.log'), 'a');
...
/**
 * MCP Server for SpecShip
 *
 * Implements the Model Context Protocol to expose SpecShip
...
export class MCPServer {
   ...
   *
   * Decision order:
   *   1. `SPECSHIP_NO_DAEMON=1` → direct mode (unchanged pre-#411 behavior).
   *   2. `SPECSHIP_DAEMON_INTERNAL=1` → we ARE the detached daemon; listen.
   *   3. No `.specship/` reachable → direct mode (the daemon's lockfile and
   *      socket both live under `.specship/`).
  ...
  async start(): Promise<void> {
    ...
    if (daemonOptOutSet()) {
      return this.startDirect('SPECSHIP_NO_DAEMON set');
    }
    ...
    if (!root) {
      ...
      return this.startDirect('no .specship/ root found');
    }
    ...
    try {
      ...
      process.stderr.write(`[SpecShip MCP] Proxy path failed (${msg}); falling back to direct mode.\n`);
  ...
  private async startDirect(reason: string): Promise<void> {
    if (reason && process.env.SPECSHIP_MCP_DEBUG) {
      process.stderr.write(`[SpecShip MCP] Direct mode: ${reason}.\n`);
    }
   ...
   * `SPECSHIP_DAEMON_INTERNAL=1`). Arbitrate the O_EXCL lock, then either
  ...
  private async startDaemonProcess(): Promise<void> {
    ...
    for (let attempt = 0; attempt < TAKEOVER_MAX_RETRIES; attempt++) {
      ...
      if (existing && existing.pid > 0 && isProcessAlive(existing.pid)) {
        process.stderr.write(
          `[SpecShip daemon] Another daemon (pid ${existing.pid}) already holds the lock; exiting.\n`
        );
        process.exit(0);
      }
    ...
    process.stderr.write('[SpecShip daemon] Could not acquire the daemon lock; exiting.\n');
    process.exit(0);
  }
  ...
  private installPpidWatchdog(): void {
    ...
    const pollMs = parsePpidPollMs(process.env.SPECSHIP_PPID_POLL_MS);
    if (pollMs <= 0) return;
    this.ppidWatchdog = setInterval(() => {
      ...
      if (ppidChanged || hostGone) {
        ...
        process.stderr.write(
          `[SpecShip MCP] Parent process exited (${reason}); shutting down.\n`
        );
        this.stop();
      }
    }, pollMs);
...
export { SpecShipPackageVersion } from './version';
