 ...
 * resources (SpecShip, watcher, ToolHandler) live in the shared
...
export class MCPSession {
   ...
   *      that were `specship init`'d *after* the server started.
   */
  private async retryInitIfNeeded(): Promise<void> {
    ...
    if (this.engine.hasDefaultSpecShip()) return;
  ...
  private async initFromRoots(): Promise<void> {
    ...
    try {
      ...
      if (rootPath) {
        ...
        process.stderr.write('[SpecShip MCP] Client returned no workspace roots; falling back to process cwd.\n');
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[SpecShip MCP] roots/list request failed (${msg}); falling back to process cwd.\n`);
    }
