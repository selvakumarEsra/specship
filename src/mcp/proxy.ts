...
export async function runProxy(
  socketPath: string,
  expectedVersion: string = SpecShipPackageVersion,
): Promise<ProxyResult> {
  ...
  if (hello.specship !== expectedVersion) {
    process.stderr.write(
      `[SpecShip MCP] Found a daemon on ${socketPath} but version (${hello.specship}) ` +
  ...
  process.stderr.write(
    `[SpecShip MCP] Attached to shared daemon on ${socketPath} (pid ${hello.pid}, v${hello.specship}).\n`
  );
...
export async function runLocalHandshakeProxy(deps: LocalHandshakeDeps): Promise<void> {
  ...
  if (socket && !shuttingDown) {
    ...
    daemonStatus = 'failed';
    process.stderr.write('[SpecShip MCP] Shared daemon unavailable; serving this session in-process (degraded).\n');
...
function startPpidWatchdogNoSocket(onDeath: () => void): void {
  const pollMs = parsePollMs(process.env.SPECSHIP_PPID_POLL_MS);
  if (pollMs <= 0) return;
  ...
  const timer = setInterval(() => {
    if (process.ppid !== originalPpid || (hostPpid !== null && !isProcessAliveLocal(hostPpid))) {
      process.stderr.write('[SpecShip MCP] Parent process exited; shutting down.\n');
      onDeath();
    }
  }, pollMs);
  timer.unref?.();
  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk: string | Buffer) => {
      ...
      try {
        ...
        if (typeof parsed.specship !== 'string' || typeof parsed.pid !== 'number') {
...
function pipeUntilClose(socket: net.Socket): Promise<void> {
  return new Promise((resolve) => {
    ...
    socket.on('error', (err) => {
      process.stderr.write(`[SpecShip MCP] daemon socket error: ${err.message}\n`);
      done();
    });
  });
}
...
function startPpidWatchdog(socket: net.Socket): void {
  ...
  const timer = setInterval(() => {
    ...
    if (ppidChanged || hostGone) {
      ...
      process.stderr.write(`[SpecShip MCP] Parent process exited (${reason}); shutting down.\n`);
      try { socket.destroy(); } catch { /* ignore */ }
      process.exit(0);
    }
  }, pollMs);
