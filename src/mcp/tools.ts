...
export class ToolHandler {
  ...
  private projectCache: Map<string, SpecShip> = new Map();
  ...
  constructor(private cg: SpecShip | null) {}
  ...
  private toolAllowlist(): Set<string> | null {
    ...
    const short = (s: string) => s.trim().replace(/^specship_/, '');
   ...
   */
  private getSpecShip(projectPath?: string): SpecShip {
    if (!projectPath) {
      if (!this.cg) {
        const searched = this.defaultProjectHint ?? process.cwd();
        throw new Error(
          'No SpecShip project is loaded for this session.\n' +
    ...
    const resolvedRoot = findNearestSpecShipRoot(projectPath);
  ...
  private worktreeMismatchFor(projectPath?: string): WorktreeIndexMismatch | null {
    ...
    try {
      mismatch = detectWorktreeIndexMismatch(startPath, this.getSpecShip(projectPath).getProjectRoot());
    } catch {
  ...
  async execute(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    try {
      ...
      switch (toolName) {
        ...
        case 'specship_spec':
          result = await handleSpecshipSpec(this.getSpecShip(args.projectPath as string | undefined), args); break;
        case 'specship_link_assert':
          result = await handleSpecshipLinkAssert(this.getSpecShip(args.projectPath as string | undefined), args); break;
  ...
  private async handleNode(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (listed.length) {
      ...
      out.push(
        '',
        `> Need one of these in full? Call specship_node again with \`file\` (e.g. \`"${listed[0]!.filePath.split('/').pop()}"\`) or \`line\` — do NOT Read it.`,
      );
    }
  ...
  private async renderNodeSection(cg: SpecShip, node: Node, includeCode: boolean): Promise<string> {
    ...
    return (
      this.formatNodeDetails(node, code, outline) +
      this.formatTrail(cg, node) +
      renderLinkedSpecsForNode(cg, node.id)
    );
  }
   ...
   */
  private formatTrail(cg: SpecShip, node: Node): string {
    const TRAIL_CAP = 12;
    ...
    const lines: string[] = ['', '### Trail — specship_node any of these to follow it (no Read needed)'];
    if (callees.length > 0) {
   ...
   */
  private findSymbolMatches(cg: SpecShip, symbol: string): Node[] {
   ...
   */
  private findAllSymbols(cg: SpecShip, symbol: string): { nodes: Node[]; note: string } {
   ...
   */
  private buildContainerOutline(cg: SpecShip, node: Node): string {
  ...
  private formatNodeDetails(node: Node, code: string | null, outline?: string | null): string {
    ...
    if (outline) {
      lines.push('', outline, '',
        `> Structural outline only. Read \`${node.filePath}\` or call specship_node on a specific member for its body.`);
    } else if (code) {
