 ...
 * the project's {@link SpecShip} instance, file watcher, and the
...
// Lazy-load the heavy SpecShip chain (sqlite + query/graph/context layers) OFF
...
const loadSpecShip = (): typeof import('../index').default =>
...
export class MCPEngine {
  private cg: SpecShip | null = null;
  private toolHandler: ToolHandler;
  // Project root we resolved to. Null until `ensureInitialized` succeeds
  // (or null forever if no .specship/ ever turned up — that's a valid
  ...
  /** Whether the default project's SpecShip is open. */
  hasDefaultSpecShip(): boolean {
    return this.toolHandler.hasDefaultSpecShip();
  }

  /**
   * Walk up from `searchFrom` to find the nearest `.specship/` and open it.
  ...
  async ensureInitialized(searchFrom: string): Promise<void> {
    ...
    if (this.toolHandler.hasDefaultSpecShip()) return;
    if (this.initPromise) {
  ...
  retryInitializeSync(searchFrom: string): void {
    ...
    const resolvedRoot = findNearestSpecShipRoot(searchFrom);
    ...
    try {
      ...
      this.cg = loadSpecShip().openSync(resolvedRoot);
      this.projectPath = resolvedRoot;
      this.toolHandler.setDefaultSpecShip(this.cg);
      this.startWatching();
      this.catchUpSync();
    } catch {
  ...
  private async doInitialize(searchFrom: string): Promise<void> {
    ...
    const resolvedRoot = findNearestSpecShipRoot(searchFrom);
    if (!resolvedRoot) {
      // No .specship/ above searchFrom. Sessions may still discover one later via roots/list
    ...
    this.projectPath = resolvedRoot;
    try {
      this.cg = await loadSpecShip().open(resolvedRoot);
      this.toolHandler.setDefaultSpecShip(this.cg);
      this.startWatching();
      this.catchUpSync();
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[SpecShip MCP] Failed to open project at ${resolvedRoot}: ${msg}\n`);
    }
  }

  /**
   * Start file watching on the active SpecShip instance. Idempotent — the
  ...
  private startWatching(): void {
    ...
    if (disabledReason) {
      process.stderr.write(
        `[SpecShip MCP] File watcher disabled — ${disabledReason}. ` +
        `The graph will not auto-update; run \`specship sync\` (or install the git sync hooks via \`specship init\`) to refresh.\n`
      );
    ...
    const debounceMs = parseDebounceEnv(process.env.SPECSHIP_WATCH_DEBOUNCE_MS);
    if (debounceMs !== undefined) {
      process.stderr.write(`[SpecShip MCP] File watcher debounce: ${debounceMs}ms (SPECSHIP_WATCH_DEBOUNCE_MS)\n`);
    }
    ...
    const started = this.cg.watch({
      ...
      onSyncComplete: (result) => {
        if (result.filesChanged > 0) {
          process.stderr.write(
            `[SpecShip MCP] Auto-synced ${result.filesChanged} file(s) in ${result.durationMs}ms\n`
          );
        }
      },
      onSyncError: (err) => {
        process.stderr.write(`[SpecShip MCP] Auto-sync error: ${err.message}\n`);
      },
    });

    this.watcherStarted = true;
    if (started) {
      process.stderr.write('[SpecShip MCP] File watcher active — graph will auto-sync on changes\n');
    } else {
      process.stderr.write(
        '[SpecShip MCP] File watcher unavailable on this platform — run `specship sync` to refresh the graph after changes.\n'
      );
    }
  }
  ...
  private catchUpSync(): void {
    ...
    const p = cg
      ...
      .then((result) => {
        ...
        if (changed > 0) {
          process.stderr.write(`[SpecShip MCP] Caught up ${changed} file(s) changed since last run\n`);
        }
      })
      .catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[SpecShip MCP] Catch-up sync failed: ${msg}\n`);
      });
...
/**
 * Parse and clamp the SPECSHIP_WATCH_DEBOUNCE_MS env override.
 *
