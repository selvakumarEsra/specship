 ...
 *   - `SPECSHIP_ASCII=1`  -> ASCII (escape hatch for any terminal)
 *   - `SPECSHIP_UNICODE=1` -> Unicode (opt-in on Windows)
...
export function supportsUnicode(): boolean {
  if (process.env.SPECSHIP_ASCII === '1') return false;
  if (process.env.SPECSHIP_UNICODE === '1') return true;
